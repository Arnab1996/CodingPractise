	...
	public int maxDistToClosest(int[] seats) {
		...
		for (int i = 0; i < numberOfSeats; i++) {
			if (i == 0 || i == numberOfSeats - 1) {
				maxDistance = Math.max(maxDistance, Math.max(distanceFromLeft[i], distanceFromRight[i]));
			} else {
				maxDistance = Math.max(maxDistance, Math.min(distanceFromLeft[i], distanceFromRight[i]));
			}
		}
