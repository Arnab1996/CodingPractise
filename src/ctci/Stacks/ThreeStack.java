...
public class ThreeStack {
	...
	public ThreeStack(int numberOfStacks, int defaultSize) {
	...
	private int adjustIndex(int index) {
		/*
		 * Java's mod operator can return neg values. For example, (-11 % 5) will return
		 * -1, not 4. We actually want the value to be 4 (since we're wrapping around
		 * the index).
		 */
	...
	private void shift(int stackNum) {
		 ...
		 * over by one element. This stack can now claim the freed index.
		 */
		if (stack.size >= stack.capacity) {
			int nextStack = (stackNum + 1) % info.length;
			shift(nextStack);
			stack.capacity++;
			// claim index that next stack lost
		}
